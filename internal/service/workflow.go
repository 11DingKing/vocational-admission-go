...
func (s AdmissionService) ClosePlan(ctx context.Context, id int64, actor int64, req string) error {
	...
	pendingStatus := domain.ApplicationReviewing
	sum, e := s.List(ctx, id, string(pendingStatus), 1, 0)
	if e != nil {
		return e
	}
	if len(sum) > 0 && pendingStatus == domain.ApplicationReviewing {
		return fmt.Errorf("%w: applications still under review", domain.ErrConflict)
	}
