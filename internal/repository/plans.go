...
func (r PlanRepo) ReserveGroupCommitted(ctx context.Context, planID, groupID int64) error {
	...
	res, e := r.DB.ExecContext(ctx, "UPDATE major_groups SET used_capacity=used_capacity+1,version=version+1 WHERE id=? AND plan_id=? AND used_capacity<capacity", groupID, planID)
	if e != nil {
		return e
	}
	n, _ := res.RowsAffected()
...
func (r PlanRepo) ReservePlanCommitted(ctx context.Context, planID int64) error {
	res, e := r.DB.ExecContext(ctx, "UPDATE plans SET used_capacity=used_capacity+1,version=version+1 WHERE id=? AND used_capacity<total_capacity", planID)
	if e != nil {
		return e
	}
	n, _ := res.RowsAffected()
